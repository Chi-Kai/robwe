...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        for i in range(args.num_clients):
            train_data = read_client_data(args.dataset, args.datadir, i, is_train=True)
            test_data = read_client_data(args.dataset, args.datadir, i, is_train=False)
            client = clientRep(args, 
                            id=i, 
        ...
        for p in prec: 
            ...
            for c in clients:
                ...
                layers_to_prune = ['head.0.weight', 'head.0.bias', 'head.2.weight', 'head.2.bias', 'head.4.weight', 'head.4.bias']
                pruning_resnet_layer(c.model,p,layers_to_prune)
                # pruning_linear_layers(c.model.head,p)
                w_acc = test_watermark(c.model,c.key['x'],c.key['b'],args.device)
                acc,num,_= c.test_metrics()
                test_acc =  acc / num  
