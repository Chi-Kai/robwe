...
class FedAvg(Server):
    def __init__(self, args, times):
        ...
        # create dataset
        self.dataset_train, self.dataset_test, self.dict_train, self.dict_test = (
            getdata(args)
        )
        self.save_data_dict()
    ...
    def evaluate(
        self, test_dataset, dict_test, train_dataset, dict_train, acc=None, loss=None
    ):
    ...
    def watermark_metrics(self):
        ...
        for c in self.selected_clients:
            w_acc = test_watermark(
                c.model, c.key["x"], c.key["b"], self.device, self.layer_type
            )
    ...
    def watermark_for_allclients(self):
        ...
        for ckey in self.clients:
            ...
            for cmodel in self.clients:
                w_acc = test_watermark(
                    cmodel.model, ckey.key["x"], ckey.key["b"], self.device, self.layer_type
                )
        ...
        tocsv(
            ...
            + "/watermark/bits"
            + str(self.args.watermark_bits)
            + "_clients"
            + str(self.args.num_clients)
            + ".csv",
            all_client_acc,
        )
    ...
    def save_global_model(self):
        ...
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        file_path = (
            model_path + "/" + "bits" + str(self.args.watermark_bits) + "_global.pt"
        )
        torch.save(self.global_model.state_dict(), file_path)

    def save_data_dict(self):
        # 保存
        save_path = self.save_folder_name + "/dataset/" + str(self.args.watermark_bits)
        if not os.path.exists(self.save_folder_name + "/dataset/"):
            os.makedirs(self.save_folder_name + "/dataset/")
        torch.save(self.dict_train, save_path + "_dataset_train.pt")
        torch.save(self.dict_test, save_path + "_dataset_test.pt")
