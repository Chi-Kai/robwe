...
def test_img_local(net_g, dataset, args,idx=None,indd=None, user_idx=-1, idxs=None):
    ...
    if leaf:
        data_loader = DataLoader(DatasetSplit_leaf(datatest_new,np.ones(len(datatest_new))), batch_size=args.batch_size, shuffle=False)
    else:
        data_loader = DataLoader(DatasetSplit(dataset,idxs), batch_size=args.batch_size,shuffle=False)
    if 'sent140' in args.dataset:
        hidden_train = net_g.init_hidden(args.batch_size)
    count = 0
    for idx, (data, target) in enumerate(data_loader):
        if 'sent140' in args.dataset:
            input_data, target_data = process_x(data, indd), process_y(target, indd)
            if args.batch_size != 1 and input_data.shape[0] != args.batch_size:
                break
