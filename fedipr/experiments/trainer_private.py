...
def test_signature(model,kwargs, ind, device):
        model.eval()
        avg_private = 0
        count_private = 0
        
        with torch.no_grad():
            if isinstance(model, (AlexNet,CNNCifar,CNNMnist,CNNCifar100)):
                for m in kwargs:
                    if kwargs[m]['flag'] == True:
                        ...
                        M = M.to(device)
                        if ind == 0 or ind == 1:
                            signbit = model.features[int(m)].scale.view([1, -1]).mm(M).sign().to(device)
                        if ind == 2 or ind == 3:
                            w = torch.mean(model.features[int(m)].conv.weight, dim=0)
                            signbit = w.view([1,-1]).mm(M).sign().to(device)
                        privatebit = b
                        privatebit = privatebit.sign().to(device)
...
class TesterPrivate(object):
    ...
    def test_signature(self, kwargs, ind):
        ...
        with torch.no_grad():
            if kwargs != None:
                if isinstance(self.model, (AlexNet,CNNCifar,CNNMnist,CNNCifar100)):
                    for m in kwargs:
                        if kwargs[m]['flag'] == True:
                            b = kwargs[m]['b']
                            M = kwargs[m]['M']
...
class TrainerPrivate(object):
    ...
    def _local_update_noback_fine(self, dataloader,local_ep, lr):
