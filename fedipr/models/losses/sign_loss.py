...
class SignLoss():
    ...
    def get_loss(self):
        self.reset()
        if isinstance(self.model, (AlexNet,CNNCifar,CNNCifar100,CNNMnist,VGG16)):
            for m in self.kwargs:
                if self.kwargs[m]['flag'] == True:
                    b = self.kwargs[m]['b']
                    M = self.kwargs[m]['M']
                    
                    b = b.to(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
                    M = M.to(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
