...
from utils.watermark import get_X, get_b, test_watermark
...
def main(args,loadpath):
    ...
    for perc in [0, 10, 20, 30, 40, 50, 60, 70, 80, 82,84,86,88,90]:
        ...
        prunedf.append(res)
    savepath = args.save_path + '/prunedf2/'+str(args.frac)+'/'+str(args.embed_dim)
...
#         round_path = os.path.join(args.save_path, f'round_{round_num}')
...
#                 if i == 10 or i == 99:
#                     w_local_dict = model.state_dict()
#                     loaded_dict = np.load(args.save_path + '/head/0.1/{}/{}/11.npy'.format(args.embed_dim, args.epochs),
#                                           allow_pickle=True).item()
#                     state_dict = {k: torch.from_numpy(v) if isinstance(v, np.ndarray) else v for k, v in
#                                   loaded_dict.items()}
#                     for key in state_dict.keys():
#                         w_local_dict[key] = state_dict[key]
#                     w_locals[i] = w_local_dict
...
#                 w_local_dict = model.state_dict()
#                 loaded_dict = np.load(args.save_path + '/head/0.1/{}/{}/{}.npy'.format(args.embed_dim, args.epochs, i),
#                                       allow_pickle=True).item()
#                 state_dict = {k: torch.from_numpy(v) if isinstance(v, np.ndarray) else v for k, v in
#                               loaded_dict.items()}
#                 for key in state_dict.keys():
...
#             res['acc_watermark'] = acc_watermark
#             res['acc_model'] = acc_test
#             print('Round: {:2d}, prec: {:3d}, acc_watermark: {: 3f}, acc_model: {: 3f}'.format(round_num, perc,
#                                                                                                  acc_watermark, acc_test))
#             prunedf.append(res)

#             df = pd.DataFrame(
#                 {'round': [round_num for _ in range(len(prunedf))],
#                  'perc': [i['perc'] for i in prunedf],
#                  'acc_watermark': [i['acc_watermark'] for i in prunedf],
#                  'acc_model': [i['acc_model'] for i in prunedf]})
#             df.to_csv(os.path.join(round_path, f'_{args.dataset}_{args.num_users}_{args.epochs}_{perc}.csv'))
