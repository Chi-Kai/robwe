...
class CNNCifar(nn.Module):
    ...
    def get_params(self):
        state_dict = self.state_dict()
        params = {k: v.cpu().numpy() for k, v in state_dict.items() if "conv" in k}
        return params
    ...
    # 归一化层参数
    def bn_params(self):
        return [self.bn1.weight, self.bn2.weight]

    def get_bn_params(self):
        state_dict = self.state_dict()
        params = {k: v.cpu().numpy() for k, v in state_dict.items() if "bn" in k}
        return params
