...
class SignLoss:
            ...
            for m in self.kwargs:
                if self.kwargs[m]["flag"] == True:
                    ...
                    if self.scheme == 2:
                        ...
                        self.loss += (
                            self.alpha
                            * F.relu(-conv_w.view([1, -1]).mm(M).mul(b.view(-1)))
                        ).sum()
                    if self.scheme == 3:
                        for i in range(b.shape[0]):
                            if b[i] == -1:
    ...
    def add_resnet_module_loss(self, sublayer, module, b, M):
        if module == "convbnrelu_1":
            ...
            conv_w = torch.mean(
                self.model.layer4[int(sublayer)].convbnrelu_1.conv.weight, dim=0
            )
        if module == "convbn_2":
            scale = self.model.layer4[int(sublayer)].convbn_2.scale
            conv_w = torch.mean(
                self.model.layer4[int(sublayer)].convbn_2.conv.weight, dim=0
            )

        if self.scheme == 0:
            self.loss += (
                self.alpha * F.relu(-scale.view([1, -1]).mm(M).mul(b.view(-1)))
            ).sum()
        ...
        if self.scheme == 2:
            self.loss += (
                self.alpha * F.relu(-conv_w.view([1, -1]).mm(M).mul(b.view(-1)))
            ).sum()
