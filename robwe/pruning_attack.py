...
def main(args,loadpath):
    ...
    for perc in [0, 10, 20, 30, 40, 50, 60, 70, 80, 82,84,86,88,90]:
        ...
        prunedf.append(res)
    savepath = args.save_path + '/prunedf3/'+str(args.frac)+'/'+str(args.embed_dim)
...
#             res['acc_watermark'] = acc_watermark
#             res['acc_model'] = acc_test 
#             print('Round: {:2d},prec: {:3d}, acc_watermark: {: 3f}, acc_model: {: 3f}'.format(round_num,perc, acc_watermark, acc_test))
#             prunedf.append(res)

#         # savepath = args.save_path + '/prunedf/' + str(args.frac) + '/' + str(args.embed_dim)
#         # if not os.path.exists(savepath):
#         #     os.makedirs(savepath)
#         df = pd.DataFrame({'round': [round_num for _ in range(len(prunedf))],'perc': [i['perc'] for i in prunedf], 'acc_watermark': [
#                         i['acc_watermark'] for i in prunedf], 'acc_model': [i['acc_model'] for i in prunedf]})
#         df.to_csv(os.path.join(round_path, f'{args.alg}_{args.dataset}_{args.num_users}_{args.epochs}_{perc}.csv'))
